                ...
                <ul class="list-group m-t">
                    @foreach($rooms as $room)
                        <li class="list-group-item">
                            @if($room['next_event'])
                                <small class="pull-right text-navy">{{ date('H:i', strtotime($room['next_event']['start_at'])) }}</small>
                            @endif
                            <span class="label {{ $room['current_event']?'label-danger':'label-primary' }}">&nbsp;</span>
                            &nbsp;{{ $room['name'] }}
                        </li>
                    @endforeach
                </ul>
