...
if (empty($cacheContent)) {
    $users = User::where('birthday', '<>', '0000-00-00')
            ->whereRaw('DATE_ADD(birthday,
                ...
                YEAR)
            BETWEEN DATE_SUB(CONCAT(DATE_ADD(CURDATE(), INTERVAL 1 DAY), \' 00:00:00\'), INTERVAL 1 DAY) AND DATE_ADD(CURDATE(), INTERVAL 60 DAY)')
            ->whereIsMember(true)
            ...
            ->limit(5)->get();
    if (count($users) > 0) {
        $cacheContent = View::make('partials.next_birthday.inner.r2024-11-07', array('users' => $users))->render();
