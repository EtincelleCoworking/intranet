...
where  subscription_user_id is null;');
    ...
    WHERE (invoice_id is null or invoice_id = 0)
        ...
        $this->daily(function () {
            Artisan::call('etincelle:renew-pending-subscriptions');
        });
            ...
            while (count($data['bookings']) > 0) {
                ...
                if ($gap > $warningDelay) {
                    ...
                    if (!$phone) {
                        ...
                        $message_content = sprintf('Bonjour, merci de bien penser à libérer la salle "%1$s" pour %4$s comme prévu, elle est réservé ensuite à %2$s. @Etincelle',
                            $data['name'],
