...
class CoffeeShopInvoiceCommand extends Command
{
    ...
    public function fire()
    {
        ...
        $sql = sprintf('SELECT user_id, concat(users.firstname, " ", users.lastname) as username, users.slug as user_slug, SUM(quantity) as pending_item_count 
                FROM coffeeshop_orders join users on users.id = coffeeshop_orders.user_id 
                WHERE (invoice_id IS NULL) AND (coffeeshop_orders.occurs_at < "%s")', $deadline);
        if ($this->option('all')) {
            $this->output->writeln('option --all');
            $users = DB::select($sql . ' GROUP BY user_id');
        } else {
            if ($this->option('user')) {
                $this->output->writeln('option --user=' . $this->option('user'));
                $users = DB::select($sql . sprintf(' AND users.id = %d GROUP BY user_id', $this->option('user')));
            } else {
    ...
    {
        return array(
            array('all', null, InputOption::VALUE_NONE, 'Take all users', null),
            array('user', null, InputOption::VALUE_REQUIRED, 'Restrict to this user ID', null),
            array('deadline', null, InputOption::VALUE_REQUIRED, 'Only considers operations before this date. Formet : YYYY-MM-DDD. Defaults : take all.', date('Y-m-d', strtotime('tomorrow'))),
            array('dry-run', null, InputOption::VALUE_NONE, 'Do not generate invoices', null),
        );
    }
